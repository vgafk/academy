...
async def get_all(group_id: int = None) -> List[api_models.Students]:
    return await get_students(group_id)
...
async def get_students(group_id: int) -> List[api_models.Students]:
    stmt = select(sql_models.Students, sql_models.StudentGroups).join(sql_models.StudentGroups)
    stmt = stmt.where(sql_models.StudentGroups.group_id == group_id) if group_id else stmt
    ...
    async with get_session() as session:
        ...
        students = [api_models.Students(**student[0].to_filter_dict(), group_id=student[1].group_id)
                    for student in students_source]
        return students
